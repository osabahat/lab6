...
public class PokerTableController {
	...
	private void initialize() {
		...
		ruleColumn.setCellValueFactory(cellData -> cellData.getValue().GetGame());
		
		// clear the rule
        showRule(null);
        
     // if the rule changes, show it
        ruleTable.getSelectionModel().selectedItemProperty().addListener(
                (observable, oldValue, newValue) -> showRule(newValue));
	...
	private void handleDraw() {
		...
		if (act.isbEvaluateHand()) {
			...
			// System.out.println("Winning Player Position: " +
